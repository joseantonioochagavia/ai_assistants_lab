...
The `insight_engine` module currently supports structured extraction from cleaned meeting transcripts. It reads markdown transcripts generated by `meeting_assistant` and returns a JSON list with one entry per meeting, including the meeting name and extracted pain points.
