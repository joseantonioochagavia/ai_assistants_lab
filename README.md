...
- `OPENAI_INSIGHT_MODEL` (optional, defaults to `gpt-4.1-mini`)
...
The `insight_engine` module currently supports structured extraction from cleaned meeting transcripts. It reads markdown transcripts generated by `meeting_assistant` and returns a JSON list with one entry per meeting, including the meeting name, extracted pain points, and relevant business themes.
